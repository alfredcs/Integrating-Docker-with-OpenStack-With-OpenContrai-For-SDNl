...
[ `rpm -qa | grep keystone|wc -l` -lt 1 ] && { yum -y install --disablerepo=* --enablerepo=${REPO_SERVERS} keystone python-keystoneclient openstack-utils; [[ $? -gt 0 ]] && { echo "YUM installation failed!"; exit 1; } }
# Configure Keystone
