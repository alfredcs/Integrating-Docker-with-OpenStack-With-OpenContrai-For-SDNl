...
[[ `rpm -qa | grep horizon|wc -l` -gt 0 ]] && rpm -e --nodeps `rpm -qa|egrep 'horizon|mod-ssl|mod-wsgi'`
yum clena all
yum -y install mod_wsgi mod_ssl
yum -y install --disablerepo=* --enablerepo=`echo ${REPO_SERVERS}|sed 's/contrail.*\,//' | sed 's/\,contrail.*//'` horizon
[[ $? -ne 0 ]] && { echo "Install Horizon failed!"; exit 1; }
[[ ! -d /etc/openstack-dashboard ]] && mkdir -p /etc/openstack-dashboard
