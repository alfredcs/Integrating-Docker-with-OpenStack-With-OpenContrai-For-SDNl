...
yum -y install --disablerepo=* --enablerepo=${REPO_SERVERS} nova python-novaclient rfc3986 pyasn1 psutil Jinja2 python-neutronclient cliff amqp gnutls-utils kombu MarkupSafe
[[ $? -ne 0 ]] && { echo "install Nova rpm failed"; exit 1; }

#####
# Configure Nova
###
useradd -M nova; usermod -L nova
[[ ! -d /etc/nova/ ]] && mkdir -p /etc/nova
[[ ! -d /var/log/nova/ ]] && mkdir -p /var/log/nova
...
[ -f ./nova/policy.json ] && cp -p ./nova/policy.json /etc/nova/policy.json
[ -f ./nova/sudoers.d.nova ] && cp -p ./nova/sudoers.d.nova /etc/sudoers.d/nova
...
# Config systemctl
####
for file_n in `ls  nova/openstack-nova*.service |sed 's/nova\///'`
do
	/bin/cp -fp nova/${file_n} /lib/systemd/system/${file_n}
	[[ -f /etc/systemd/system/${file_n} ]] && unlink /etc/systemd/system/${file_n}
	ln -s /lib/systemd/system/${file_n} /etc/systemd/system/${file_n}
done
