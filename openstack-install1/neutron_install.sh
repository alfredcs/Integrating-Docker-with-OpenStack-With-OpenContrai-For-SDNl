...
####
[[ `rpm -qa | grep neutron |wc -l` -gt 0 ]] && rpm -e --nodeps neutron python-neutronclient alembic Mako
yum -y install --disablerepo=* --enablerepo=`echo ${REPO_SERVERS}|sed 's/contrail.*\,//' | sed 's/\,contrail.*//'` neutron python-neutronclient alembic Mako
[[ $? -ne 0 ]] && { echo "Neutron installation failed!"; exit 1; }
