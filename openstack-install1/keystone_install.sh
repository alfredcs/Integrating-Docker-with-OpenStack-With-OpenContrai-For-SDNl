...
[[ `rpm -qa | egrep -i 'percona|mysql'|wc -l` -gt 0 ]] && { systemctl stop mysql@bootstrap.service; kill -9 `ps -ef | grep -v grep|grep mysql | awk '{print $2}'`; rpm -e --nodeps `rpm -qa | grep -i percona`; rpm -e --nodeps `rpm -qa | grep -i mysql`; rm -rf /var/lib/mysql /var/run/mysqld /var/lock/subsys/mysqld; }
###
# Configure mysql
##
...
yum -y  --disablerepo=* --enablerepo=CentOS7* --enablerepo=percona_install_repo --enablerepo=${REPO_SERVERS}  install  Percona-XtraDB-Cluster-server-56 Percona-XtraDB-Cluster-client-56 Percona-XtraDB-Cluster-galera-3 #Percona-Server-shared-56
[[ $? != 0 ]] && { echo "MySql install failed!"; exit 1; }
###
# Install keystone
###
[[ `rpm -qa | grep -ei 'python-babel|Babel'| wc -l` -gt 0 ]] && rpm -e --nodeps `rpm -qa | grep -ei 'python-babel|Babel'`
for dep_list in  Routes SQLAlchemy WebOb deploy dogpile.cache dogpile.core eventlet greenlet iso8601 jsonschema monkey oauthlib oslo.concurrency oslo.config oslo.context oslo.db oslo.i18n oslo.log oslo.messaging oslo.middleware oslo.policy oslo.serialization oslo.six oslo.utils passlib pbr pycadf python-netaddr python-paste-deploy pytz repoze repoze.lru six stevedore cryptography enum cffi python-msgpack MySQL-python ciff cmd2 pycparser pyparsing netifaces cliff-tablib Babel argparse
do
	[[ `rpm -qa | grep -i ${dep_list}|wc -l` -gt 0 ]] && rpm -e --nodeps `rpm -qa | grep -i "python-${dep_list}"`
	yum -y install --disablerepo=* --enablerepo=${REPO_SERVERS} ${dep_list}
done
[ `rpm -qa | grep keystone|wc -l` -lt 1 ] && { yum -y install --disablerepo=* --enablerepo=${REPO_SERVERS} keystone python-keystoneclient openstack-utils crudini sqlalchemy-migrate sqlparse python-memcached memcached
[[ $? -gt 0 ]] && { echo "YUM installation failed!"; exit 1; } }
# Configure Keystone
...
[[ ! -d /etc/keystone/ssl/private/ ]] && mkdir -p /etc/keystone/ssl/private/
[[ ! -d /etc/keystone/ssl/certs/ ]] && mkdir -p /etc/keystone/ssl/cert2/
...
if [[ ${first_openstack_controller} == ${local_controller} ]] ; then
	...
	[[ `rpm -qa | grep python-paste|wc -l` -lt 1 ]] && yum install python-paste python-paste-deploy
...
useradd -M keystone; usermod -L keystone

####
# Setup startup for keystone
###

[[ -f ./keystone/openstack-keystone.service ]] && /bin/cp -p ./keystone/openstack-keystone.service /lib/systemd/system/openstack-keystone.service
[[ -f /lib/systemd/system/openstack-keystone.service ]] && ln -s /lib/systemd/system/openstack-keystone.service /etc/systemd/system/openstack-keystone.service
[[ `uname -r` =~ ^3\. ]] &&  { systemctl enable openstack-keystone.service; systemctl enable memcached.service; systemctl restart memcached.service; systemctl restart openstack-keystone.service; }
